...
Shader "Maki/Realtime Clock" {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata v) { 
				// float4 offsetTimeTexture = tex2Dlod(_MainTex,float4(0,0,0,0));
				// offsetTimeTexture.rgb = LinearToGammaSpace(offsetTimeTexture.rgb);

				// int3 offsetTime = int3(
				// 	round(offsetTimeTexture.r*24), // 0 to 24
				// 	round(offsetTimeTexture.g*60), // 0 to 60
				// 	round(offsetTimeTexture.b*60)  // 0 to 60
				// );
